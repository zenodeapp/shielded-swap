...
CHOICE_3="3. Shield $DENOM1_NAMADA or $DENOM2_NAMADA (from '$NAM_TRANSPARENT' to '$NAM_VIEWING_KEY')"
CHOICE_4="3. Unshield $DENOM1_NAMADA or $DENOM2_NAMADA (from '$NAM_TRANSPARENT' to '$NAM_VIEWING_KEY'; only works if chain is not broken)"
CHOICE_BACK="Back"
...
if [ "$MENU_CHOICE" = "$CHOICE_1" ] || [ "$MENU_CHOICE" = "$CHOICE_2" ]; then
  ...
  echo ""
  header_block "SHIELD/UNSHIELD AN ASSET"
  ...
  if ! [ "$SHIELDED_BROKEN" = 'true' ]; then
    CONFIRM_SS=$(gum confirm "You may need to perform a shielded sync before doing this (especially if your viewing key is new). Do you want to perform a shielded sync before swapping?" && echo "true" || echo "false")
